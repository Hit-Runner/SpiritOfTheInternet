...
class insertWindow:
    def __init__(self, dbIns):
        ...
        self.customWindow.title("Insert")  # 窗口名称
        ...
        self.s_snoEntry = tkinter.Entry(self.sOperationFrame, textvariable=self.s_snoVar, width=10)
        ...
        self.sButton = tkinter.Button(self.sOperationFrame, text="插入", font=self.customFont, width=8,
                                      height=1, command=self.instS)
        ...
        self.c_cnoEntry = tkinter.Entry(self.cOperationFrame, textvariable=self.c_cnoVar, width=5)
        ...
        self.cButton = tkinter.Button(self.cOperationFrame, text="插入", font=self.customFont, width=5,
                                      height=1, command=self.instC)
        ...
        self.sc_snoEntry = tkinter.Entry(self.scOperationFrame, textvariable=self.sc_snoVar, width=8)
        ...
        self.sc_cnoEntry = tkinter.Entry(self.scOperationFrame, textvariable=self.sc_cnoVar, width=8)
        ...
        self.scButton = tkinter.Button(self.scOperationFrame, text="插入", font=self.customFont, width=5,
                                       height=1, command=self.instSC)
    ...
    def instS(self):
        try:
            if self.dbIns is None:
                ...
                sno = self.s_snoVar.get()
                sclass = self.s_sclassVar.get()
                ...
                if sno == "":
                    ...
                    self.dbIns.openCursor()  # 打开游标
                    result = self.dbIns.insertCustom(
                        "s",
                        "(" + sclass + "," + sno + "," + repr(sname) + "," + repr(ssex) + "," + sage + "," +
                        repr(sdept) + ")"
                    )
                    if result is True:
                        self.alert("插入学生成功")
    ...
    def instSC(self):
        try:
            if self.dbIns is None:
                ...
                sno = self.sc_snoVar.get()
                sclass = self.sc_sclassVar.get()
                cno = self.sc_cnoVar.get()
